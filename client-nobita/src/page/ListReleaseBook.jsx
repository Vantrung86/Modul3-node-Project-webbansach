...
function ListReleaseBook() {
  ...
  const getData = async ()=>{
    try {
      ...
      let res = response.data.filter((e)=>e.catergoryId === 3)
      setDatas(res)
    } catch (error) {   
      console.log(error);
    }
  ...
  const handleSort = (sort) => {
    if (sort == "1") {
      let result = datas.sort((a,b)=>a.price - b.price)
      setDatas(result)
    }else if(sort == "2"){
      let result = datas.sort((a,b)=>-a.price + b.price)
      setDatas(result)
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        <div className="path">
          <p>
            Nobita.vn - Nhà Sách Trên Mạng /<span> Sắp phát hành</span>
          </p>
        </div>
          ...
          {displayedProducts.map((element, index) => (
            <div className="listnewbook_decoration" key={element.productId}>
                <Link to={`/bookdetail/${element.productId}`}>
                  ...
                  <p className="listnewbook-name">{element.nameProduct}</p>
...
export default ListReleaseBook;
